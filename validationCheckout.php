	...
	<div class="container-fluid">
		...
		if (mysqli_num_rows($sql) < 1) {
			...
			if (mysqli_num_rows($sql_staf) < 1) {
				...
				# PENGECEKAN MAHASISWA TIDAK TERJADWAL
				...
				if (mysqli_num_rows($free_access) < 1) {
					...
					# CHECKING REDUNDANT ACCESS
					if (Query::checkRedundant($date_now, $input_qr, 'check out', $conn)) {
					...
					<?php Query::insertLog($row['nim'], 'check out', 'tidak terjadwal', $conn);
			...
			# PENGECEKAN MAHASISWA TERDJADWAL
			if (Query::checkRedundant($date_now, $input_qr, 'check out', $conn)) {
			...
			<?php Query::insertLog($row['nim'], 'check out', 'terjadwal', $conn);
